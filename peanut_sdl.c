...
int main(int argc, char **argv) {
    ...
    window = SDL_CreateWindow("2024 GameBoy: Opening File",
    ...
    switch (argc) {
        case 1:
            SDL_SetWindowTitle(window, "Drag Gameboy File Here!!");
            do {
                SDL_Delay(10);
                SDL_PollEvent(&event);
...
SDL_LogMessage(LOG_CATERGORY_PEANUTSDL,
...

    {
        ...
        char title_str[28] = "2023GameBoy:";
        printf("ROM: %s\n", gb_get_rom_name(&gb, title_str + 12));
        printf("MBC: %d\n", gb.mbc);
        SDL_SetWindowTitle(window, title_str);
    }

    SDL_SetWindowMinimumSize(window, LCD_WIDTH*4, LCD_HEIGHT*4);
    ...
    /* Use integer scale. */
    SDL_RenderSetLogicalSize(renderer, LCD_WIDTH*4, LCD_HEIGHT*4);
